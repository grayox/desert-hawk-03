...
* implemented in current version, will carry forward
† skipped so far in current version, come back to later
‡ skipped in current version, no plan to return, delete in next version
...
‡ 14. roll back items 9 through 13 because causes problem reading `fuse.themes` from
      src/@fuse/components/FuseTheme/FuseTheme.js
  ...
  16. src/firebaseService/firebaseService.js
      ...
      modify to read/write from Cloud Firestore instead of Realtime Database
        - consult src/config/firebaseConfig.js
        ````
  ...
  38. src/store.js
      ...
      //    a. Right-click an object in Chrome's console and select Store as Global Variable from the contextmenu.   It       will return something like temp1 as the variable name.
...
11. /src/firebase-db/index.js -- `export firebase` because that is where `firebase.initializeApp()` happens;
   ...
   - then add `firebase` as argument to read: `reactReduxFirebase(firebase, AppConfig.firebaseConfig),`
12. Above item numbered 11 might no longer be necessary.
